...
public class CloseResourceRule extends AbstractJavaRule {
    ...
    private void checkForResources(ASTMethodOrConstructorDeclaration methodOrConstructor, Object data) {
        ...
        for (Map.Entry<ASTVariableDeclarator, TypeNode> resVarEntry : resVars.entrySet()) {
            ...
            } else if (isReassignedBeforeBeingClosed(resVar, methodOrConstructor)) {
                reportedVarNames.add(resVar.getVariableId().getName());
                addViolationWithMessage(data, resVar, reassignBeforeClosedMessageForVar(resVar.getName()));
            }
        }
    }
    ...
    private boolean isReassignedBeforeBeingClosed(ASTVariableDeclarator variable, ASTMethodOrConstructorDeclaration methodOrConstructor) {
    ...
    private boolean isAssignmentForVariable(ASTStatementExpression statement, ASTVariableDeclarator variable) {
        NameDeclaration statementVariable = statement.getFirstDescendantOfType(ASTName.class).getNameDeclaration();
